<?php
/*
 * Main Gateway of traaittPlatform using either a local daemon or the explorer
 * Authors: Serhack, cryptochangements, mosu-forge
 */

defined( 'ABSPATH' ) || exit;

require_once('class-traaittplatform-service.php');

class traaittPlatform_Gateway extends WC_Payment_Gateway
{
    private static $_id = 'traaittplatform_gateway';
    private static $_title = 'traaittPlatform Gateway';
    private static $_method_title = 'traaittPlatform Gateway';
    private static $_method_description = 'traaittPlatform Gateway Plug-in for WooCommerce.';
    ...
    private static $use_traaittplatform_price = false;
    private static $use_traaittplatform_price_decimals = TRAAITTPLATFORM_GATEWAY_ATOMIC_UNITS;

    private static $traaittplatform_service;
    private static $log;
    ...
    {
        return apply_filters('woocommerce_gateway_icon', '<img src="'.TRAAITTPLATFORM_GATEWAY_PLUGIN_URL.'assets/images/traaittplatform-icon.png"/>', $this->id);
    }

    function __construct($add_action=true)
    {
        $this->id = self::$_id;
        $this->method_title = __(self::$_method_title, 'traaittplatform_gateway');
        $this->method_description = __(self::$_method_description, 'traaittplatform_gateway');
        ...
        self::$address = $this->settings['traaittplatform_address'];
        ...
        self::$use_traaittplatform_price = $this->settings['use_traaittplatform_price'] == 'yes';
        self::$use_traaittplatform_price_decimals = $this->settings['use_traaittplatform_price_decimals'];

        $explorer_url = TRAAITTPLATFORM_GATEWAY_EXPLORER_URL;

        if($add_action)
            add_action('woocommerce_update_options_payment_gateways_'.$this->id, array($this, 'process_admin_options'));

        // Helper functions
        self::$traaittplatform_service = new traaittplatform_Library(self::$host, self::$port, self::$password);
        self::$log = new WC_Logger();
    }

    public function init_form_fields()
    {
        $this->form_fields = include 'admin/traaittplatform-gateway-admin-settings.php';
    }
    ...
    public function admin_options()
    {
        ...
        include TRAAITTPLATFORM_GATEWAY_PLUGIN_DIR . '/templates/traaittplatform-gateway/admin/settings-page.php';
    }
    ...
    public static function admin_balance_info()
    {
        ...
        $wallet_amount = self::$traaittplatform_service->getBalance();
        ...
        if (!isset($wallet_amount)) {
            ...
            return array(
                'height' => $height,
                'balance' => self::format_traaittplatform($wallet_amount['lockedAmount']).' ETRX',
                'unlocked_balance' => self::format_traaittplatform($wallet_amount['availableBalance']).' ETRX'
            );
        }
    }
    ...
    public function process_payment($order_id)
    {
        ...
        $traaittplatform_amount = 1e8 * $fiat_amount / $rate;

        if(self::$discount)
            $traaittplatform_amount = $traaittplatform_amount - $traaittplatform_amount * self::$discount / 100;

        // only use whole numbers for ETRX
        $traaittplatform_amount = intval($traaittplatform_amount);
        
        $traaittplatform_amount = intval($traaittplatform_amount * TRAAITTPLATFORM_GATEWAY_ATOMIC_UNITS_POW);

        $query = $wpdb->prepare("INSERT INTO $table_name (order_id, payment_id, currency, rate, amount) VALUES (%d, %s, %s, %d, %d)", array($order_id, $payment_id, $currency, $rate, $traaittplatform_amount));
        $wpdb->query($query);

        $order->update_status('on-hold', __('Awaiting offline payment', 'traaittplatform_gateway'));
    ...
    public static function do_update_event()
    {
        ...
        // Get Live Price
        $ETRX_link = 'https://tradeogre.com/api/v1/ticker/ltc-ETRX';
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_URL => $ETRX_link,
        ));
        $resp = curl_exec($curl);
        curl_close($curl);
        $price_ETRX = json_decode($resp, true);
        
        $api_link = 'https://min-api.cryptocompare.com/data/price?fsym=LTC&tsyms=USD&extraParams=ETRX_woocommerce';
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_RETURNTRANSFER => 1,
        ...
        if($price_ETRX['success'] === true && isset($price_ETRX['bid'])) {
            if(isset($price['USD'])) {
                ...
                $rate_ltc = round($price_ETRX['bid'] * 1e8);
        ...
        $height = self::$traaittplatform_service->getHeight();
        set_transient('traaittplatform_gateway_network_height', $height);

        // Get pending payments
        $table_name_1 = $wpdb->prefix.'traaittplatform_gateway_quotes';
        $table_name_2 = $wpdb->prefix.'traaittplatform_gateway_quotes_txids';
        ...
        foreach($pending_payments as $pending) {
            ...
            $amount_traaittplatform = $quote->amount_total;
            ...
            $paid = $amount_paid > $amount_traaittplatform - TRAAITTPLATFORM_GATEWAY_ATOMIC_UNIT_THRESHOLD;
            ...
            if($paid && $confirmed) {
                self::$log->add('traaittPlatform_Payments', "[SUCCESS] Payment has been confirmed for order id $order_id and payment id $payment_id");
                ...
                if(self::is_virtual_in_cart($order_id) == true){
                    $order->update_status('completed', __('Payment has been received.', 'traaittplatform_gateway'));
                } else {
                    $order->update_status('processing', __('Payment has been received.', 'traaittplatform_gateway'));
                }

            } else if($paid) {
                self::$log->add('traaittPlatform_Payments', "[SUCCESS] Payment has been received for order id $order_id and payment id $payment_id");
                ...
                if($order_age_seconds > self::$valid_time) {
                    self::$log->add('traaittPlatform_Payments', "[FAILED] Payment has expired for order id $order_id and payment id $payment_id");
                    $query = $wpdb->prepare("UPDATE $table_name_1 SET pending=0 WHERE payment_id=%s", array($payment_id));
                    $wpdb->query($query);

                    unset(self::$payment_details[$order_id]);

                    $order->update_status('cancelled', __('Payment has expired.', 'traaittplatform_gateway'));
                }
            }
        }
    }
    ...
    {
        $txs = array();
        $payments = self::$traaittplatform_service->get_all_payments($payment_id);
        ...
        if (count($details)) {
            ...
            foreach($details as $tx) {
                ...
                $txs[] = array(
                    ...
                    'amount_formatted' => self::format_traaittplatform($tx->amount_paid)
                );
            ...
            if(count($heights) && !in_array(0, $heights)) {
                $height = get_transient('traaittplatform_gateway_network_height');
                $highest_block = max($heights);
            ...
            $time_to_confirm = self::format_seconds_to_time($blocks_to_confirm * TRAAITTPLATFORM_GATEWAY_DIFFICULTY_TARGET);
            ...
            $array_integrated_address = self::$traaittplatform_service->make_integrated_address($address, $payment_id);
            if (isset($array_integrated_address['integratedAddress'])) {
                $integrated_addr = $array_integrated_address['integratedAddress'];
            } else {
                self::$log->add('traaittPlatform_Gateway', '[ERROR] Unable get integrated address');
            ...
            $qrcode_uri = 'traaittplatform:'.$address.'?tx_amount='.$amount_due.'&tx_payment_id='.$payment_id;
            ...
            $payment_details = array(
                ...
                'amount_total_formatted' => self::format_traaittplatform($amount_total),
                'amount_paid_formatted' => self::format_traaittplatform($amount_paid),
                'amount_due_formatted' => self::format_traaittplatform($amount_due),
                'status' => $status,
    ...
    public static function get_payment_details_ajax() {
        ...
        if($order->get_payment_method() != self::$_id)
            self::ajax_output(array('error' => '[ERROR] Order not paid for with traaittPlatform'));
    ...
    public static function admin_order_page($post)
    {
        ...
        if(!is_array($details)) {
            $error = $details;
            include TRAAITTPLATFORM_GATEWAY_PLUGIN_DIR . '/templates/traaittplatform-gateway/admin/order-history-error-page.php';
            return;
        }
        include TRAAITTPLATFORM_GATEWAY_PLUGIN_DIR . '/templates/traaittplatform-gateway/admin/order-history-page.php';
    }
    ...
    public static function customer_order_page($order)
    {
        ...
        if(!is_array($details)) {
            $error = $details;
            include TRAAITTPLATFORM_GATEWAY_PLUGIN_DIR . '/templates/traaittplatform-gateway/customer/order-error-page.php';
            return;
        }
        $show_qr = self::$show_qr;
        $details_json = json_encode($details);
        $ajax_url = WC_AJAX::get_endpoint('traaittplatform_gateway_payment_details');
        include TRAAITTPLATFORM_GATEWAY_PLUGIN_DIR . '/templates/traaittplatform-gateway/customer/order-page.php';
    }
    ...
    public static function customer_order_email($order)
    {
        ...
        $details = self::get_payment_details($order_id);
        if(!is_array($details)) {
            include TRAAITTPLATFORM_GATEWAY_PLUGIN_DIR . '/templates/traaittplatform-gateway/customer/order-email-error-block.php';
            return;
        }
        include TRAAITTPLATFORM_GATEWAY_PLUGIN_DIR . '/templates/traaittplatform-gateway/customer/order-email-block.php';
    }
    ...
    public static function use_traaittplatform_price()
    {
        return self::$use_traaittplatform_price;
    }
    ...
    {
        $rate = self::get_live_rate($currency);
        $traaittplatform_amount = intval(TRAAITTPLATFORM_GATEWAY_ATOMIC_UNITS_POW * 1e8 * $price / $rate) / TRAAITTPLATFORM_GATEWAY_ATOMIC_UNITS_POW;
        $traaittplatform_amount_formatted = number_format($traaittplatform_amount, self::$use_traaittplatform_price_decimals);

        return <<<HTML
            <span class="woocommerce-Price-amount amount" data-price="$price" data-currency="$currency"
        data-rate="$rate" data-rate-type="live">
            $traaittplatform_amount_formatted
            <span class="woocommerce-Price-currencySymbol">ETRX</span>
        </span>
...
HTML;
    ...
    public static function format_traaittplatform($atomic_units) {
        return sprintf(TRAAITTPLATFORM_GATEWAY_ATOMIC_UNITS_SPRINTF, $atomic_units / TRAAITTPLATFORM_GATEWAY_ATOMIC_UNITS_POW);
    }
